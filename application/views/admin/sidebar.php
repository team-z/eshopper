<div id="sidebar">
			<div id="sidebar-wrapper">
				<div class="sidebar-title">
					<span class="text-size-40 text-loose">E-shopper</span><br>
					<span class="text-size-18 text-grey">Loe Pasti puaass</span>
				</div>
				<div class="sidebar-avatar">
					<div class="sidebar-avatar-image"><img src="<?php echo base_url('assets/images/me.PNG'); ?> " alt="" class="img-circle"></div>
					<div class="sidebar-avatar-text">e-shopper</div>
				</div>
				<ul class="sidebar-nav">
					<li class="sidebar-close"><a href="#"><i class="fa fa-fw fa-close"></i></a></li>
					<li><a href="<?php echo base_url('index.php/admin/index'); ?>"><i class="fa fa-fw fa-star"></i><span>Dashboard</span></a></li>
					<li><a href="<?php echo base_url('index.php/admin/viewbarang'); ?>"><i class="fa fa-archive fa-font"></i><span>Barang</span></a></li>
					<li><a href="<?php echo base_url('index.php/admin2/viewtransaksi'); ?>"><i class="fa fa-credit-card fa-list-alt"></i><span>Transaksi</span></a></li>
					<li><a href="<?php echo base_url('index.php/admin4/view_pembelian'); ?>"><i class="fa fa-shopping-basket"></i><span>Pembelian</span></a></li>
					<li><a href="<?php echo base_url('index.php/admin3/join'); ?>"><i class="fa fa-fw fa-truck"></i><span>daftar pengiriman</span></a></li>
				</ul>
				<div class="sidebar-footer">
					<hr style="border-color: #DDD">
					created by <a href="#" target="_blank" class="text-default">Team-Z</a><br>
				</div>
			</div>
		</div>